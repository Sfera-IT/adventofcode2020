...
fn parse_line(y: i64, line: &str, grid: &mut HashSet<(i64, i64, i64)>) {
    for (x, c) in line.chars().enumerate() {
        if c == '#' {
            grid.insert((x as i64, y, 0));
        }
    }
}

fn evolve(grid: &mut HashSet<(i64, i64, i64)>) {
    let mut to_be_removed = HashSet::new();
    let mut to_be_inserted = HashMap::new();

    for (x,y,z) in grid.iter().copied() {
        ...
        if neighbours != 2 && neighbours != 3 {
            to_be_removed.insert((x, y, z));
        }
    }

    grid.retain(|p| !to_be_removed.contains(p));

    for ((x, y, z), n) in to_be_inserted.iter() {
        if *n == 3 {
            grid.insert((*x, *y, *z));
        }
    }
