...
impl Ferry {
    ...
    fn at(&self, x: isize, y: isize) -> Seat {
        if x < 0 || y < 0 || x >= (self.width as isize) || y >= (self.height as isize) {
            Seat::Corridor
        } else {
            self.layout[(y as usize) * self.width + (x as usize)]
        }
    }
    ...
    fn evolve_single(&self, x: usize, y: usize) -> (Seat, bool) {
        ...
        for dy in -1..=1 {
            for dx in -1..=1 {
                if (dx != 0 || dy != 0) && self.at(x + dx, y + dy) == Seat::Occupied {
                    occupied += 1;
                }
            }
        }
        ...
        let new = match (cur, occupied) {
            ...
            (Seat::Occupied, o) if o >= 4 => Seat::Empty,
            (s, _) => s,
        };
